...
public class ObjectDetectionSample extends LinearOpMode {
    ...
    public void runOpMode() throws InterruptedException {
        ...
        if (robot.limelightCameras != null && robot.limelightCameras.length > 0) {
            robot.limelightCameras[0].getLimelight().pipelineSwitch(1);
            robot.limelightCameras[0].getLimelight().start();
        }
        currentPipeline = 1;
        ...
        while (opModeIsActive() && !isStopRequested()) {
            ...
            if (gamepad1.b && currentPipeline != 0) {
                if (robot.limelightCameras != null && robot.limelightCameras.length > 0) {
                    robot.limelightCameras[0].getLimelight().pipelineSwitch(0);
                }
                currentPipeline = 0;
                Thread.sleep(200);
            }
        ...
        // Cleanup
        if (robot.limelightCameras != null && robot.limelightCameras.length > 0) {
            robot.limelightCameras[0].getLimelight().pipelineSwitch(0);  // Back to AprilTag
        }
    }
    ...
    private boolean isTargetDetected() {
        ...
        com.qualcomm.hardware.limelightvision.LLResult result = robot.limelightCameras[0].getLimelight().getLatestResult();
